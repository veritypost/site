...
type KidProfileRow = Pick<
  Database['public']['Tables']['kid_profiles']['Row'],
  'id' | 'display_name' | 'avatar_color' | 'streak_current'
>;

export type KidExitIntent = 'leave' | 'switch';

export interface KidChromeContextValue {
  openExitPin: (intent?: KidExitIntent) => void;
  hasActiveKid: boolean;
}

export interface KidTopChromeProps {
  children: ReactNode;
}
...
const KidChromeContext = createContext<KidChromeContextValue>({
  openExitPin: () => {},
  hasActiveKid: false,
});

export function useKidChrome(): KidChromeContextValue {
  return useContext(KidChromeContext);
}

function readActiveKid(): string | null {
  if (typeof window === 'undefined') return null;
  try { return window.localStorage.getItem(ACTIVE_KID_KEY) || null; } catch { return null; }
}

const KidTopChrome: FC<KidTopChromeProps> = ({ children }) => {
  const router = useRouter();
  const [activeKidId, setActiveKidId] = useState<string | null>(() => readActiveKid());
  const [kid, setKid] = useState<KidProfileRow | null>(null);

  const [exitIntent, setExitIntent] = useState<KidExitIntent | null>(null);
  ...
  const [pinNotSet, setPinNotSet] = useState<boolean>(false);
  const modalRef = useRef<HTMLDivElement | null>(null);

  useEffect(() => {
    const sync = () => setActiveKidId(readActiveKid());
    window.addEventListener('vp:kid-mode-changed', sync);
    const onStorage = (e: StorageEvent) => { if (e.key === ACTIVE_KID_KEY) setActiveKidId(readActiveKid()); };
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data } = await supabase
        ...
        .maybeSingle();
      if (!cancelled) setKid((data as KidProfileRow | null) || null);
    })();
  ...
  const verifyExitPin = useCallback(async (candidate: string) => {
  ...
  const openExitPin = useCallback((intent?: KidExitIntent) => {
    if (!kid) return;
...
const TopBar: FC<TopBarProps> = ({ kid, onExit }) => {
...
const StreakChip: FC<StreakChipProps> = ({ days }) => {
  const hot = days >= 3;
...
const ExitPinModal: FC<ExitPinModalProps> = ({
    ...
    >
      ...
      >
        ...
        <input
          ...
          onChange={(e: ChangeEvent<HTMLInputElement>) => onPinChange(e.target.value)}
          placeholder="----"
