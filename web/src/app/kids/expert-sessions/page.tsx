...
type ExpertUser = { username?: string | null; display_name?: string | null };
type ExpertCategory = { name?: string | null };
type SessionRow = Pick<
  Tables<'kid_expert_sessions'>,
  'id' | 'title' | 'scheduled_at' | 'duration_minutes'
> & { users?: ExpertUser | null; categories?: ExpertCategory | null };
...
export default function KidExpertSessions() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!hasPermission('kids.expert.list_sessions')) {
        setDenied(true);
        setLoading(false);
        return;
      }
