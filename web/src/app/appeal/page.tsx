...
export default function AppealPage() {
  ...
  async function submitAppeal(id: string) {
    ...
    if (!text) {
      setError('Tell us why this was wrong.');
      return;
    }
    setError('');
    setFlash('');
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px 80px' }}>
      ...
      <p style={{ fontSize: 13, color: '#666', marginTop: 0, lineHeight: 1.5 }}>
        If a moderator took action on your account and you think it was a mistake, you can file one
        appeal per penalty. Approved appeals reverse the penalty.
      </p>
      ...
      {warnings.length === 0 ? (
        ...
        warnings.map((w) => {
          ...
          const statusColor =
            status === 'approved' ? '#16a34a' : status === 'denied' ? '#dc2626' : '#b45309';
          return (
            <div
              key={w.id}
              style={{
