...
type BookmarkRow = Tables<'bookmarks'> & {
  articles: (Pick<Tables<'articles'>, 'id' | 'title' | 'slug' | 'excerpt' | 'published_at'> & {
...
export default function BookmarksPage() {
  ...
  async function load() {
    ...
    const { data: bms, error: bmsErr } = await supabase
      .from('bookmarks')
      .select('id, notes, created_at, collection_id, articles!fk_bookmarks_article_id(id, title, slug, excerpt, published_at, categories!fk_articles_category_id(name))')
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
          ...
          {(canExport || canCollections) && (
            <div style={{ display: 'flex', gap: 8 }}>
              {canExport && <button onClick={exportAll} style={btnGhost}>Export JSON</button>}
              {canCollections && <button onClick={() => setShowNewCollection(true)} style={btnSolid}>+ Collection</button>}
            </div>
          )}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {filtered.length === 0 && (
            <div style={{ textAlign: 'center', padding: '60px 20px', color: '#666' }}>
              <div style={{ fontSize: 15, fontWeight: 600, color: '#111', marginBottom: 6 }}>No bookmarks yet</div>
              <div style={{ fontSize: 13, lineHeight: 1.5, maxWidth: 360, margin: '0 auto' }}>Save articles here. Tap the bookmark icon on any story to come back later.</div>
              <a href="/browse" style={{ display: 'inline-block', padding: '10px 20px', background: '#111', color: '#fff', borderRadius: 10, fontSize: 13, fontWeight: 600, textDecoration: 'none', marginTop: 12 }}>Browse articles</a>
            </div>
          )}
        </div>
      </div>
...
const btnSolid: CSSProperties = { padding: '8px 14px', background: '#111', color: '#fff', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: 'pointer' };
const btnGhost: CSSProperties = { padding: '8px 14px', background: 'transparent', color: '#111', border: '1px solid #e5e5e5', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer' };
