  ...
  if (!canView) {
    return (
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '20px 16px 80px' }}>
        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 24, textAlign: 'center', color: C.dim }}>
      ...
      {(!application || application.status === 'rejected') && canApply && (
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 18 }}>
          ...
          <p style={{ fontSize: 13, color: C.dim, marginTop: 0, lineHeight: 1.5 }}>
            Expert, Educator, and Journalist badges are the only public authority signals on the platform.
            Applications are reviewed editorially - you will submit 3 sample responses that two editors score independently.
            Approved applicants enter a 30-day probation where responses are reviewed before publishing.
          </p>

          <Field label="I am applying as">
            <select value={form.application_type} onChange={e => setForm({ ...form, application_type: e.target.value as ExpertFormState['application_type'] })} style={inputStyle}>
          ...
          <div style={{ fontSize: 12, color: C.dim, marginBottom: 10 }}>Pick real questions you have been asked in your field and answer them the way you would answer in the app.</div>
...
function StatusCard({ tone, title, children }: { tone: 'success' | 'warn' | 'danger'; title: string; children: ReactNode }) {
  ...
  return (
    <div style={{ background: picked.bg, border: `1px solid ${picked.border}`, borderRadius: 12, padding: '14px 18px', marginBottom: 18 }}>
