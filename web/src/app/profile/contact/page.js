  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch('/api/support', {
        ...
        body: JSON.stringify({ category: topic, subject: subject.trim(), description: body.trim() }),
      });
      if (!res.ok) {
  ...
  return (
    <div className="vp-dark">
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '20px 16px 80px' }}>
        ...
        <h1 style={{ fontSize: 22, fontWeight: 700, margin: '0 0 6px', letterSpacing: '-0.02em' }}>Contact Us</h1>
        ...
        {submitted ? (
          <div style={{
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--white)', marginBottom: 8 }}>Message sent</div>
            <div style={{ fontSize: 13, color: 'var(--dim)', marginBottom: 16 }}>
              We'll get back to you as soon as possible.
            </div>
            <button
              onClick={() => { setSubmitted(false); setTopic(''); setSubject(''); setBody(''); }}
              style={{
                padding: '10px 20px', borderRadius: 8, border: '1px solid var(--border)',
                background: 'var(--bg)', color: 'var(--white)', fontSize: 13, fontWeight: 600,
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {/* Topic */}
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--dim)', marginBottom: 6 }}>Topic</label>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {TOPICS.map(t => (
                  <button
                    key={t.value}
                    onClick={() => { setTopic(t.value); if (!subject || TOPICS.some(x => x.label === subject)) setSubject(t.value === 'other' ? '' : t.label); }}
                    style={{
            ...
            {/* Subject */}
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--dim)', marginBottom: 6 }}>Subject</label>
              <input
                value={subject}
            ...
            {/* Body */}
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--dim)', marginBottom: 6 }}>Message</label>
              <textarea
                value={body}
            ...
            <button
              ...
              style={{
                ...
                background: !submitting && topic && subject.trim() && body.trim() ? '#111111' : '#cccccc',
                color: '#ffffff',
                fontSize: 14, fontWeight: 600, cursor: !submitting && topic && subject.trim() && body.trim() ? 'pointer' : 'not-allowed',
