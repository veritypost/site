...
export default function FamilyDashboard() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const achievementsOk = hasPermission('family.shared_achievements') || hasPermission('kids.achievements.view');
      ...
      const [lb, ach, rep] = await Promise.all([
        leaderboardOk
          ? fetch('/api/family/leaderboard', { credentials: 'include' }).then(r => r.json()).catch(() => ({}))
          : Promise.resolve({}),
        achievementsOk
          ? fetch('/api/family/achievements', { credentials: 'include' }).then(r => r.json()).catch(() => ({}))
          : Promise.resolve({}),
        reportOk
          ? fetch('/api/family/weekly-report', { credentials: 'include' }).then(r => r.json()).catch(() => ({}))
          : Promise.resolve({}),
      ]);
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 16px 80px' }}>
      <a href="/profile/kids" style={{ fontSize: 12, color: C.dim, textDecoration: 'none' }}>&larr; Kids</a>
            ...
            {achievements.length === 0 ? (
              ...
              return (
                <div key={a.id} style={{
                  ...
                  {earned ? (
                    <span style={{ fontSize: 11, color: C.success, fontWeight: 700 }}>Earned {a.earned_at ? new Date(a.earned_at).toLocaleDateString() : ''}</span>
                  ) : (
