...
function tierLabel(tier: string | null): string | null {
  ...
  // Defensive arm for Owner Mode tier strings. The analytics-side userTier
  // label can be 'ownermode' (see NavWrapper.deriveTier); the author-side
  // `plans.tier` will never be 'ownermode' in current DB shape because no
  // plan row holds that tier, but adding the explicit arm keeps the badge
  // readable if the value ever flows here through some future client-derived
  // path. Mirror verity_pro's compact label.
  if (tier === 'ownermode') return 'pro';
