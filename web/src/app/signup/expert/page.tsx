...
export default function ExpertSignupPage() {
  ...
  const toggleExpertise = (f: string) => setExpertise((prev) => prev.includes(f) ? prev.filter((x) => x !== f) : [...prev, f]);
  ...
  return (
    <div style={{
      ...
      <div style={{ backgroundColor: C.card, border: `1px solid ${C.border}`, borderRadius: '18px', padding: '40px 36px', width: '100%', maxWidth: '480px', boxSizing: 'border-box' }}>
        ...
        {isAuthed && email && (
          <div style={{ marginBottom: '18px' }}>
            ...
            <input type="email" value={email} readOnly
              style={{ ...field('email-ro'), backgroundColor: '#f3f4f6', color: C.dim, cursor: 'not-allowed' }}
              aria-readonly="true" />
          </div>
        )}
        ...
        {!authChecked && (
          <p style={{ fontSize: '13px', color: C.dim, textAlign: 'center', padding: '20px 0' }}>Loading…</p>
        )}
        ...
        <p style={{ textAlign: 'center', fontSize: '13px', color: C.dim, marginTop: '20px' }}>
          {isAuthed ? (
            <>
              Changed your mind?{' '}
              <a href="/profile/settings" style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.accent, fontWeight: 600, fontSize: '13px', padding: 0, fontFamily: 'inherit', textDecoration: 'none' }}>
                Back to settings
              </a>
            </>
          ) : (
