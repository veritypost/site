...
function buildSuggestions(email: string | null | undefined, displayName: string | null | undefined): string[] {
...
export default function PickUsernamePage() {
  ...
  const checkName = async (name: string): Promise<Availability> => {
    ...
    const [{ data: takenRow }, { data: reservedRow }] = await Promise.all([
      supabase.from('users').select('username').eq('username', name).maybeSingle<{ username: string }>(),
      supabase.from('reserved_usernames').select('username').eq('username', name).maybeSingle<{ username: string }>(),
    ]);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const { error: updateError } = await supabase
        .rpc('update_own_profile', { p_fields: { username } });
      if (updateError) {
