...
function Masthead() {
  return (
    <div style={{ padding: '20px 20px 14px', borderBottom: `1px solid ${T.rule}` }}>
      ...
      >
        {EDITION_DATE.toUpperCase()} \u00b7 {EDITION_TIME.toUpperCase()} \u00b7 {STORIES.length}{' '}
        STORIES
      </div>
    </div>
  );
...
export default function SpreadPrototype() {
  ...
  const tail = STORIES.filter(
    (s) => s.role !== 'lead' && s.role !== 'subordinate' && s.role !== 'sidebar'
  ).slice(0, 8);
    ...
    >
      <div style={{ maxWidth: 560, margin: '0 auto', textAlign: 'center', marginBottom: 32 }}>
        ...
        <div style={{ fontSize: 13, color: T.textDim, lineHeight: 1.55 }}>
          Small-multiples map. Entire edition visible in one screen. The brief notes this paradigm
          strains the dek-legibility invariant \u2014 measure drops to ~22 characters per line in
          the two-column tail.
        </div>
      </div>
