  ...
  return (
    <div style={{ padding: '14px 24px 18px', borderTop: `1px solid ${T.ruleSoft}` }}>
      ...
      <div style={{ fontSize: 11, color: T.textMute, fontFamily: T.sans, letterSpacing: '0.02em' }}>
        ...
        {story.hoursAgo && story.hoursAgo < 24 && <span> \u00b7 {story.hoursAgo}h ago</span>}
      </div>
    </div>
  );
}
...
export default function BriefingPrototype() {
  ...
  const today = STORIES.filter(
    (s) => !s.sinceLastVisit && s.role !== 'sidebar' && (s.hoursAgo || 0) < 24
  );
  const evergreen = STORIES.filter(
    (s) => !s.sinceLastVisit && s.role !== 'sidebar' && (s.hoursAgo || 0) >= 24
  );
    ...
    >
      <div style={{ maxWidth: 560, margin: '0 auto', textAlign: 'center', marginBottom: 32 }}>
        ...
        <div style={{ fontSize: 13, color: T.textDim, lineHeight: 1.55 }}>
          Email-tray paradigm. Three named zones: Since you last visited \u00b7 Today\u2019s edition
          \u00b7 Still worth your time. Conditional breaking strip on top.
        </div>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <PhoneFrame>
          ...
          <ZoneHead
            ...
            count={today.length}
            note="The editor\u2019s argument for the day."
          />
          {today.map((s) => (
            <StoryRow key={s.id} story={s} />
          ))}
          ...
          <ZoneHead
            label="Still worth your time"
            count={evergreen.length}
            note="Pieces from the last 72 hours you may have missed."
          />
          {evergreen.map((s) => (
            <StoryRow key={s.id} story={s} dense />
          ))}

          <div style={{ padding: '32px 24px 80px', textAlign: 'center' }}>
            <div
              style={{ fontSize: 11, color: T.textMute, fontFamily: T.serif, fontStyle: 'italic' }}
            >
              \u2014 end of briefing \u2014
            </div>
          </div>
