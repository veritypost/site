...
export async function PATCH(request, { params }) {
  try { await requirePermission('admin.ads.campaigns.edit'); }
  catch (err) {
  ...
  if (error) return safeErrorResponse(NextResponse, error, { route: 'admin.ad_campaigns.id', fallbackStatus: 400 });
  return NextResponse.json({ ok: true });
}

export async function DELETE(_request, { params }) {
  try { await requirePermission('admin.ads.campaigns.delete'); }
  catch (err) {
    if (err.status) return NextResponse.json({ error: err.message }, { status: err.status });
    return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
  }
