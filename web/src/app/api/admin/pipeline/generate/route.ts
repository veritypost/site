...
${catListText}`;
    ...
    const summaryUser = `Write a plain-text summary in 1–2 sentences, 18–22 words total (target ≈120 characters). The summary must convey what actually happened — not a tease, not a hook — but stay tight enough that an article card reading it on a 320px mobile viewport never has to clamp or truncate. Must not restate the headline. Must contain different facts than the headline. Reader who sees only the summary must know the news. Return JSON with ONLY a "summary" field: {"summary":"<your summary>"}. Respond with the JSON object only — no preamble, no markdown fence. Today: ${new Date()
      .toISOString()
      .slice(0, 10)}.${freeformBlock}\n\nSOURCES:\n${corpus}`;
