...
const CRON_NAME = 'purge-audit-log';

// T352 — runs nightly. Calls purge_audit_log() which hard-deletes rows
// older than 365 days. Pairs with anonymize-audit-log-pii (90d soft) and
// the audit_log_created_at_idx for query performance.
...
async function run(request) {
  ...
  try {
    ...
    const { data, error } = await service.rpc('purge_audit_log');
    if (error) {
      await logCronHeartbeat(CRON_NAME, 'error', { error: error.message });
      return safeErrorResponse(NextResponse, error, {
        route: 'cron.purge_audit_log',
        fallbackStatus: 500,
      });
    }
