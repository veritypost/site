...
export async function GET(request) {
  ...
  let user;
  try { user = await requirePermission('notifications.inbox.view'); }
  ...
  let q = service.from('notifications').select('*').eq('user_id', user.id)
    .order('created_at', { ascending: false }).limit(limit);
  if (unread) q = q.eq('is_read', false);

  const { data, error } = await q;
  if (error) return safeErrorResponse(NextResponse, error, { route: 'notifications.get', fallbackStatus: 400 });

  const { count } = await service.from('notifications')
...
export async function PATCH(request) {
  const blocked = await v2LiveGuard(); if (blocked) return blocked;
  let user;
  try { user = await requirePermission('notifications.mark_read'); }
  catch (err) { if (err.status) return NextResponse.json({ error: err.message }, { status: err.status }); return NextResponse.json({ error: 'Internal error' }, { status: 500 }); }

  const { ids, mark, all } = await request.json().catch(() => ({}));
  const service = createServiceClient();
  const now = new Date().toISOString();
  const update = mark === 'seen'
  ...
  const { error } = await q;
  if (error) return safeErrorResponse(NextResponse, error, { route: 'notifications.patch', fallbackStatus: 400 });
