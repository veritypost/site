...
export async function POST(request) {
  ...
  if (error) {
    ...
    // T283 + T16 + T17 — collapse USER_NOT_FOUND / DM_PAID_PLAN /
    // DM_MUTED / DM_RECIPIENT_OPTED_OUT / DM_BLOCKED into a single
    // `cannot_dm` 403. Distinguishing them by status code lets a caller
    // enumerate which user_ids exist (404 vs 403), which are on a paid
    // tier vs blocked, which recipients have toggled allow_messages off
    // (T16), and which counterparties are mutually blocked (T17). Keep
    // the granular reason in server logs only; the client-facing
    // surface is uniform. Self-message and missing-ids are caller-input
    // shape errors and stay as 400 — they don't leak target-user state.
    let status;
    let userMsg;
    if (
      code === 'DM_PAID_PLAN' ||
