...
type LeaderUser = Pick<
  Tables<'users'>,
  'id' | 'username' | 'avatar_url' | 'avatar_color'
  | 'is_verified_public_figure' | 'is_expert' | 'verity_score' | 'streak_current'
  | 'quizzes_completed_count' | 'articles_read_count' | 'comment_count'
> & { displayScore?: number };
...
interface CategoryScoreRow {
  ...
  users: Pick<
    ...
    | 'is_verified_public_figure' | 'is_expert' | 'verity_score' | 'streak_current'
    | 'quizzes_completed_count' | 'articles_read_count' | 'comment_count'
    | 'email_verified' | 'is_banned' | 'show_on_leaderboard' | 'frozen_at'
  >;
}
    ...
    (async () => {
      ...
      if (authRes.data?.user) {
        const { data: meRow } = await supabase
          .from('users')
          .select('id, username, avatar_url, avatar_color, is_verified_public_figure, is_expert, verity_score, streak_current, quizzes_completed_count, articles_read_count, comment_count, email_verified, plan_status')
    ...
    async function load() {
      ...
      if (activeCat) {
        const { data: csRows } = await supabase
          ...
          .select('user_id, score, users!inner ( id, username, avatar_url, avatar_color, is_verified_public_figure, is_expert, verity_score, streak_current, quizzes_completed_count, articles_read_count, comment_count, email_verified, is_banned, show_on_leaderboard, frozen_at )')
      ...
      if (activeTab === 'Rising Stars') {
        ...
        const { data } = await supabase
          .from('users')
          .select('id, username, avatar_url, avatar_color, is_verified_public_figure, is_expert, verity_score, streak_current, quizzes_completed_count, articles_read_count, comment_count')
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 16, overflowX: 'auto', scrollbarWidth: 'none' }}>
            {TABS.filter(t => fullAccess || t === 'Top Verifiers').map(t => (
              <button key={t} onClick={() => { setActiveTab(t); setActiveCat(null); setActiveSub(null); }} style={{
                padding: '12px 18px', borderRadius: 20, border: 'none', minHeight: 44,
