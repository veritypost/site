...
export default function RhStyles() {
  const css = `
    ...
    /* 2026-05-18 — .vp-rh-search* and .vp-rh-fpill* rules moved to
       app/globals.css so the controls render correctly when mounted in
       the global NavWrapper header on every page (article, profile,
       /search, etc.), not just the home. The home-scoped overrides
       (.vp-rh-masthead .vp-rh-search { ... }) stay here because
       .vp-rh-masthead only exists on the home shell. */
    ...
    /* 2026-05-18 — .vp-rh-masthead2*, .vp-rh-search__explore, and
       .vp-rh-fpill* rules moved to app/globals.css. The controls now
       mount in the global NavWrapper header on every page; their CSS
       must always be loaded, not gated on the home shell rendering
       RhStyles. */
  `;
