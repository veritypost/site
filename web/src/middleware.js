...
function buildCsp(nonce) {
  ...
  return [
    ...
    `connect-src 'self' ${supabaseOrigin} ${supabaseWss} https://api.stripe.com https://api.openai.com https://*.ingest.sentry.io https://www.google-analytics.com https://analytics.google.com https://stats.g.doubleclick.net https://region1.google-analytics.com https://www.googletagmanager.com`
      .replace(/\s+/g, ' ')
      .trim(),
...
// Ext-OO.2 — strict Report-Only header (no unsafe-inline in style-src)
// was removed 2026-04-30. The profile shell uses heavy inline styles;
// on iPhone Safari the flood of per-element violation reports caused
// ~20k function invocations in 5 minutes. Diagnostic result: the
// profile/settings components need a CSS-class migration before this
// policy can be enforced. Re-add once that migration ships.
...
export async function middleware(request) {
  ...
  const betaGateAllowed =
    ...
    pathname === '/terms' ||
    pathname === '/privacy' ||
    pathname === '/redesign/preview' ||
    pathname === '/redesign/leaderboard' ||
