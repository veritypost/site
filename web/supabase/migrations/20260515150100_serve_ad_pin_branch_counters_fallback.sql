-- Wave 2 — serve_ad rewrite (§9): VOLATILE, pin branch, counter-backed caps,
-- fallback ladder, mandatory source field on every return path.
-- Plus AFTER INSERT trigger on ad_impressions to maintain ad_freq_counters.
...
COMMENT ON FUNCTION public.serve_ad(text, uuid, uuid, uuid) IS
  'Resolves ad creative for a placement. VOLATILE because pin lookup observes live state. Returns jsonb with mandatory ''source'' field: no_placement | editorial_block | tier_hidden | pinned | programmatic | network_fallback | no_fill.';

------------------------------------------------------------
-- Counter-increment trigger on ad_impressions
------------------------------------------------------------
...
DROP TRIGGER IF EXISTS trg_bump_ad_freq_counters ON public.ad_impressions;
CREATE TRIGGER trg_bump_ad_freq_counters
  AFTER INSERT ON public.ad_impressions
  FOR EACH ROW EXECUTE FUNCTION public._bump_ad_freq_counters();
