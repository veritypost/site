-- Item 11a Phase 2 — god-mode short-circuits in permission resolver RPCs.
--
-- Only two functions need direct patching:
--   • my_permission_keys  — root key resolver; everything else cascades from it
--   • compute_effective_perms — admin permissions console reads granted_via;
--       does NOT call my_permission_keys, so needs its own god-mode arm
--
-- Functions that cascade automatically (no patch needed):
--   • has_permission      — SELECT EXISTS(my_permission_keys...) → cascades ✓
--   • has_permission_for  — calls has_permission → cascades ✓
--   • get_my_capabilities — calls my_permission_keys internally → cascades ✓
--
-- God-mode detection: walks user_permission_sets + role_permission_sets directly.
-- Avoids calling compute_effective_perms (would be recursive) and avoids calling
-- my_permission_keys (which is the function being patched).
--
-- god_mode_keys bypasses: is_banned, requires_verified, feature_flag checks.
-- God-mode is NOT applied inside kid sessions (active_kid IS NULL guard) since
-- the owner's god-mode grant is on their adult account only.

BEGIN;

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. my_permission_keys
--    Adds god_mode_keys CTE. If caller has admin.god_mode, every active
--    non-kid-scoped permission key is unioned into the result set.
-- ─────────────────────────────────────────────────────────────────────────────
...
AS $function$
  ...
  -- God-mode short-circuit: return every active permission key when the caller
  -- has admin.god_mode and is NOT in a kid session.
  -- Bypasses banned / email-verified / feature-flag gates deliberately.
  god_mode_keys AS (
    SELECT p.key
    FROM permissions p
    WHERE p.is_active = true
  ...
  UNION
  SELECT key::varchar FROM god_mode_keys;
$function$;


-- ─────────────────────────────────────────────────────────────────────────────
-- 2. compute_effective_perms
--    Adds is_god CTE (non-recursive — walks grants directly).
--    Prepends WHEN ig.value THEN ... arm to the granted / granted_via /
--    source_detail CASE expressions so the admin permissions console shows
--    granted_via='god_mode' rather than 'none' for god-mode users.
-- ─────────────────────────────────────────────────────────────────────────────
...
AS $function$
WITH
  -- Non-recursive god-mode detection: walk grants directly rather than calling
  -- compute_effective_perms (which is this function) or my_permission_keys.
  is_god AS (
    SELECT EXISTS(
      SELECT 1
      FROM user_permission_sets ups
...
SELECT
  ...
  CASE
    WHEN ig.value
      THEN jsonb_build_object('god_mode', true)
  ...
  f.requires_verified,
  f.lock_message
FROM final f CROSS JOIN is_god ig
