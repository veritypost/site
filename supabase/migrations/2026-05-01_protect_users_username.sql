-- Defense-in-depth: lock users.username at the trigger layer too.
--
-- The update_own_profile RPC enforces the lock for the documented write path,
-- but it uses SECURITY DEFINER and therefore bypasses BEFORE UPDATE triggers.
-- This trigger catches any future writer (PostgREST, raw UPDATE, new RPC)
-- that bypasses update_own_profile but still runs as the user role.
--
-- Existing bypasses at the top of users_protect_columns
-- (app.auth_sync GUC, service_role, is_admin_or_above) short-circuit before
-- this block, so the legitimate paths (auth_sync, item 12 admin endpoint via
-- service-role, admin direct edits) all sail through. The only writer this
-- catches is end-user PostgREST/RPC writes — exactly the intended scope.
--
-- Source preserved verbatim from
-- supabase/migrations/2026-04-29_combined_unapplied.sql:792-902 with one new
-- block added before RETURN NEW.
...
BEGIN
  ...
  -- Username lock (item 10): once a non-empty username is set, only admins
  -- and service-role (handled by the early bypasses above) can rename.
  -- Protects against any future writer that bypasses update_own_profile.
