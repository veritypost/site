-- ---------------------------------------------------------------------------
-- Follow-up article generation
-- Adds optional existing_story_id to persist_generated_article so a new
-- article can be attached to an existing story instead of creating a fresh one.
-- ---------------------------------------------------------------------------
...
AS $function$
  DECLARE
    ...
    v_existing_story_id  uuid    := nullif(p_payload->>'existing_story_id','')::uuid;
    v_slug_base          text;
    v_slug               text;
    v_attempt            int     := 0;
    v_article_id         uuid;
  ...
  BEGIN
    ...
    IF v_existing_story_id IS NOT NULL THEN
      -- Attach to existing story; skip creation
      SELECT s.id, s.slug INTO v_story_id, v_slug
        FROM public.stories s WHERE s.id = v_existing_story_id;
      IF NOT FOUND THEN
        RAISE EXCEPTION 'persist_generated_article: existing_story_id % not found', v_existing_story_id
          USING ERRCODE = '22023';
      END IF;
    ELSE
