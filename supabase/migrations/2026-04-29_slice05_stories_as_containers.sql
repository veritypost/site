-- Slice 05: Stories as containers
--
-- Creates public.stories as the top-level entity. Articles gain story_id FK.
-- Slug moves from articles to stories. Timelines parent FK changes from
-- article_id to story_id and gains type + linked_article_id columns.
-- Comments gain a convenience story_id FK for queries.
--
-- persist_generated_article is rewritten to:
--   D1: skip timeline entries with unparseable event_date (WHERE guard)
--   D4: persist type='event' on all pipeline timeline entries
--   D5: insert stories row first; article gets story_id instead of slug
--   D6: insert one type='article' timeline entry per generated article
--
-- Timelines are TRUNCATED (test data only at this stage).
...
CREATE POLICY "stories_published_visible"
  ON public.stories
  FOR SELECT
  USING (published_at IS NOT NULL);

-- -----------------------------------------------------------------------
-- 2. Add story_id to articles (nullable — existing rows have no story)
-- -----------------------------------------------------------------------
ALTER TABLE public.articles
  ADD COLUMN story_id uuid REFERENCES public.stories(id) ON DELETE SET NULL;

-- -----------------------------------------------------------------------
-- 3. Timelines: TRUNCATE, drop article_id, add story_id + type + linked_article_id
-- -----------------------------------------------------------------------
TRUNCATE TABLE public.timelines;

ALTER TABLE public.timelines DROP COLUMN article_id;

ALTER TABLE public.timelines
  ADD COLUMN story_id         uuid NOT NULL REFERENCES public.stories(id) ON DELETE CASCADE,
  ADD COLUMN type             text NOT NULL DEFAULT 'event'
                                CHECK (type IN ('event', 'article')),
  ADD COLUMN linked_article_id uuid NULL REFERENCES public.articles(id) ON DELETE SET NULL;

CREATE INDEX timelines_story_id_idx ON public.timelines (story_id);

ALTER TABLE public.timelines ENABLE ROW LEVEL SECURITY;

CREATE POLICY "timelines_readable_if_story_published"
  ON public.timelines
  FOR SELECT
  USING (
    EXISTS (
...
-- -----------------------------------------------------------------------
-- 4. Comments: add convenience story_id FK
-- -----------------------------------------------------------------------
ALTER TABLE public.comments
  ADD COLUMN story_id uuid NULL REFERENCES public.stories(id) ON DELETE CASCADE;

-- -----------------------------------------------------------------------
-- 5. Drop slug from articles (lives on stories now)
-- -----------------------------------------------------------------------
ALTER TABLE public.articles DROP COLUMN slug;

-- -----------------------------------------------------------------------
-- 6. persist_generated_article — rewritten for Slice 05
--    Return type gains story_id column — must DROP before recreating.
-- -----------------------------------------------------------------------
...
CREATE OR REPLACE FUNCTION public.persist_generated_article(p_payload jsonb)
  RETURNS TABLE(article_id uuid, story_id uuid, slug text, audience text)
  LANGUAGE plpgsql
  SECURITY DEFINER
...
AS $function$
  DECLARE
    ...
    v_tags               text[]  := CASE WHEN jsonb_typeof(p_payload->'tags') = 'array'
      THEN array(SELECT jsonb_array_elements_text(p_payload->'tags')) ELSE NULL END;
    v_seo_keywords       text[]  := CASE WHEN jsonb_typeof(p_payload->'seo_keywords') = 'array'
      THEN array(SELECT jsonb_array_elements_text(p_payload->'seo_keywords')) ELSE NULL END;
  ...
  BEGIN
    ...
    -- Slug derivation (now owned by stories)
    ...
    IF v_slug_base IS NULL THEN
      v_slug_base := 'article-' || substr(
        replace(coalesce(v_run_id::text, gen_random_uuid()::text), '-', ''), 1, 8);
    END IF;
    v_slug := left(v_slug_base, 80);

    -- D5: insert story first (slug + title live here)
    <<slug_loop>>
    WHILE v_attempt < 3 LOOP
      BEGIN
        INSERT INTO public.stories (slug, title)
        VALUES (v_slug, v_title)
    ...
    -- D5: article now gets story_id, not slug
    INSERT INTO public.articles (
      story_id, title, subtitle, body, body_html, excerpt, category_id, status,
    ...
    VALUES (
      v_story_id, v_title, v_subtitle, v_body, v_body_html, v_excerpt, v_category_id, 'draft',
      true, v_ai_provider, v_ai_model, now(), v_ai_provider, v_ai_model, v_prompt_fingerprint,
      v_source_feed_id, v_source_url, v_cluster_id, v_word_count, v_reading_min,
      v_tags, v_seo_keywords, v_seo_title, v_seo_description, v_metadata,
      v_is_kids_safe,
    ...
    -- D1: WHERE guard skips entries with unparseable event_date
    -- D4: type='event' for all pipeline-sourced timeline rows
    INSERT INTO public.timelines (
      story_id, type, title, description, event_date, event_label,
      event_body, event_image_url, source_url, sort_order, metadata
    )
    SELECT
      v_story_id, 'event',
      t->>'title', t->>'description',
      public.parse_timeline_event_date(t->>'event_date'),
      coalesce(nullif(t->>'event_label',''), 'Event'),
      t->>'event_body', t->>'event_image_url', t->>'source_url',
    ...
    -- D6: one type='article' entry links this article into the story timeline
    INSERT INTO public.timelines (story_id, type, linked_article_id, event_label, event_date, sort_order)
    VALUES (v_story_id, 'article', v_article_id, v_title, now(), 999);

    -- Quizzes remain article-level
