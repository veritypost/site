-- Session D verification — make persist_generated_article lenient with
-- partial timeline event dates. The current RPC casts
-- `t->>'event_date'` directly to timestamptz, which crashes the entire
-- persist transaction whenever the model returns a non-ISO partial
-- date like "2013" for events whose exact day is unknown ("the company
-- was founded in 2013"). Strict casting at the DB boundary is wrong:
-- timeline entries are inherently noisy historical context and the
-- model legitimately produces year-only or year-month values.
--
-- Fix: introduce parse_timeline_event_date(text) that tries the direct
-- cast first, falls back to "{YYYY}-01-01", then "{YYYY-MM}-01", then
-- NULL. The persist RPC's existing `coalesce(..., now())` continues to
-- supply now() when nothing parses, preserving the timeline NOT NULL
-- contract. Result: "2013" persists as 2013-01-01 instead of aborting
-- the whole article.
--
-- Surfaced by run id b25531ef-8b8d-4207-8505-0b2b9d4e50a4 (adult
-- generate, full pipeline succeeded LLM-side, persist failed).
...
BEGIN
  ...
  -- Try direct cast (handles full ISO + most Postgres-recognized forms).
  BEGIN
  ...
  -- Year-only ("2013") -> Jan 1 of that year.
  ...
  -- Year-month ("2013-05" or "2013-5") -> 1st of that month.
...
-- Re-emit persist_generated_article with the lenient parser swapped in
-- on the timeline INSERT. Body otherwise identical to the live
-- definition.
