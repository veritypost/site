-- schema/125_rollback_124_f7_drop_kids_summary_from_rpc.sql
-- 2026-04-22 — Rollback for schema/124_f7_drop_kids_summary_from_rpc.sql
--
-- Restores the exact migration-118 RPC body (v_kids_summary DECLARE + the
-- IF-guarded UPDATE kid_articles block). The restored body still has the
-- latent footgun documented in 124's header — this rollback is only for the
-- rare case where the code path that CALLS persist_generated_article ends up
-- needing to write kids_summary and we temporarily need the RPC to accept it
-- again before a proper column-add migration lands.
--
...
BEGIN
  ...
  ELSE
    -- Rolled-back kid branch: reinstated v_kids_summary UPDATE block.
    -- This is the buggy version — kid_articles has no kids_summary column;
    -- the column-add migration must land BEFORE any caller populates the field
    -- or the transaction will roll back.
