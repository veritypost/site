-- schema/118_f7_persist_generated_article.sql
-- 2026-04-22 — F7 Phase 3 Task 13: persist generated article draft + sources + timeline + quiz
--
-- Drafted + reviewed via 4-agent pre-impl flow on 2026-04-22. Every finding
-- resolved before implementation.
--
-- STAGED pending owner apply (per §3i — no mcp apply_migration in this session).
--
-- Rollback: schema/119_rollback_118_persist_generated_article.sql
--
-- Adds:
--   public.persist_generated_article(jsonb) — SECURITY DEFINER RPC that
--     writes a single transactional draft article across articles + sources +
--     timelines + quizzes (or kid_articles + kid_sources + kid_timelines +
--     kid_quizzes for kid audience). Returns (article_id, slug, audience).
--
--   Behavior:
--     - Validates audience in ('adult','kid'), body + body_html non-empty,
--       category_id present. Raises SQLSTATE 22023 on missing fields.
--     - Builds a URL-safe slug from the title; falls back to
--       'article-<hash>' if empty or unicode-only. On unique_violation
--       retries up to 3× with a hex millisecond suffix.
--     - Inserts status='draft' — admin publish action promotes later
--       (F7 Phase 3 invariant: AI-generated content never auto-publishes).
--     - Strips `is_correct` from each quiz option; stores correct_index in
--       quizzes.metadata so the public API never leaks answer keys.
--     - Kid path populates kid_articles.kids_summary and writes child rows
--       into the kid_* tables. kid_quizzes.retention_policy defaults to
--       'delete_on_parent_request_or_12mo_inactive' (COPPA).
--     - Service-role only execute grant (mirrors pipeline_today_cost_usd).
--
-- Pre-flight verified against live DB (2026-04-22):
--   - Column names confirmed via information_schema: no `summary` column on
--     articles/kid_articles — short summary lives on `excerpt`, long on
--     metadata->'summary'.
--   - `body_html` is nullable in DB, but this RPC enforces non-empty at
--     the application layer (F7 Phase 3 sanitized-HTML invariant).
--   - articles_slug_key (UNIQUE btree(slug)) and kid_articles_slug_uniq
--     (UNIQUE btree(slug)) both exist — the unique_violation retry path
--     covers both audiences.
--   - persist_generated_article(jsonb) does not yet exist in pg_proc.
--   - `timelines.event_date` + `event_label` are NOT NULL; RPC provides
--     now() / 'Event' fallbacks if caller payload omits them.
--   - `kid_quizzes.retention_policy` DEFAULT is already the canonical
--     COPPA string — omitted from INSERT so default wins.
...
BEGIN
  ...
  ELSE
    -- Kid audience: kid_articles.kids_summary + kid_* child tables.
    -- kid_quizzes.retention_policy is omitted so the COPPA default wins.
    IF v_kids_summary IS NOT NULL THEN
      UPDATE public.kid_articles
         SET kids_summary = v_kids_summary
...
-- Newsroom generation runs on the service client; no user-scoped session
-- ever calls this RPC.
