...
-- ------------------------------------------------------------
-- role_permissions — REMOVED 2026-04-18 (Phase 5, Track N)
-- Legacy table superseded by the dot-namespaced permissions catalog
-- and permission_set_permissions / role_permission_sets bindings.
-- Dropped in migration drop_role_permissions_table_2026_04_18.
-- ------------------------------------------------------------
...
-- role_permissions FKs removed 2026-04-18 (Phase 5, Track N — table dropped).
...
-- role_permissions indexes removed 2026-04-18 (Phase 5, Track N — table dropped).
...
-- role_permissions UNIQUE constraint removed 2026-04-18 (Phase 5, Track N — table dropped).
...
-- role_permissions RLS enable removed 2026-04-18 (Phase 5, Track N — table dropped).
...
-- role_permissions RLS policies removed 2026-04-18 (Phase 5, Track N — table dropped).
...
-- Round 6: tightened. admins see all; users only overrides scoped to themselves.
CREATE POLICY "pso_select" ON "permission_scope_overrides" FOR SELECT USING (
  public.is_admin_or_above()
...
TRUNCATE TABLE
  ...
  comments, comment_votes, bookmarks, reading_log, follows,
...
-- Missing prod tables + modern RPCs + Round 6 ACL lockdown
-- Appended here so a fresh Supabase project that runs this file
-- top-to-bottom matches production schema (113 tables, current RPCs).
-- All sections idempotent: IF NOT EXISTS / CREATE OR REPLACE / DROP+CREATE
-- ============================================================

-- ------------------------------------------------------------
-- score_events (Scoring ledger) — originally added in 022_phase14_scoring.sql
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- error_logs — originally added in 030_phase22_error_logs.sql
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- user_push_tokens — originally added in 037_user_push_tokens.sql
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- admin_audit_log — originally added in 055_admin_audit_log.sql
-- ------------------------------------------------------------
...
-- ============================================================
-- MODERN RPCs (Round 4–Round 9)
-- ============================================================

-- ------------------------------------------------------------
-- handle_new_auth_user + on_auth_user_created trigger (Round 4 extension)
-- Creates public.users row + assigns owner role if first user, else user.
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.handle_new_auth_user()
RETURNS trigger
LANGUAGE plpgsql
...
-- ------------------------------------------------------------
-- reject_privileged_user_updates (Round 4 + INSERT extension v2)
-- SECURITY INVOKER so current_user reflects caller role; matches prod.
-- ------------------------------------------------------------
...
CREATE TRIGGER trg_users_reject_privileged_updates
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.reject_privileged_user_updates();

-- ------------------------------------------------------------
-- bump_user_perms_version (hardened SECDEF + auth gate)
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.bump_user_perms_version(p_user_id uuid)
RETURNS void
LANGUAGE plpgsql
...
-- ------------------------------------------------------------
-- update_own_profile (Round 5 Item 2 — single server-side write contract)
-- ------------------------------------------------------------
...
BEGIN
  ...
  SET
    username = CASE WHEN p_fields ? 'username' THEN NULLIF(p_fields->>'username', '')::varchar ELSE u.username END,
    display_name = CASE WHEN p_fields ? 'display_name' THEN NULLIF(p_fields->>'display_name', '')::varchar ELSE u.display_name END,
    bio = CASE WHEN p_fields ? 'bio' THEN (p_fields->>'bio')::varchar ELSE u.bio END,
    avatar_url = CASE WHEN p_fields ? 'avatar_url' THEN (p_fields->>'avatar_url')::text ELSE u.avatar_url END,
    avatar_color = CASE WHEN p_fields ? 'avatar_color' THEN (p_fields->>'avatar_color')::varchar ELSE u.avatar_color END,
    banner_url = CASE WHEN p_fields ? 'banner_url' THEN (p_fields->>'banner_url')::text ELSE u.banner_url END,
    profile_visibility = CASE WHEN p_fields ? 'profile_visibility' THEN (p_fields->>'profile_visibility')::varchar ELSE u.profile_visibility END,
    show_activity = CASE WHEN p_fields ? 'show_activity' THEN (p_fields->>'show_activity')::boolean ELSE u.show_activity END,
    show_on_leaderboard = CASE WHEN p_fields ? 'show_on_leaderboard' THEN (p_fields->>'show_on_leaderboard')::boolean ELSE u.show_on_leaderboard END,
    allow_messages = CASE WHEN p_fields ? 'allow_messages' THEN (p_fields->>'allow_messages')::boolean ELSE u.allow_messages END,
    dm_read_receipts_enabled = CASE WHEN p_fields ? 'dm_read_receipts_enabled' THEN (p_fields->>'dm_read_receipts_enabled')::boolean ELSE u.dm_read_receipts_enabled END,
    notification_email = CASE WHEN p_fields ? 'notification_email' THEN (p_fields->>'notification_email')::boolean ELSE u.notification_email END,
    notification_push = CASE WHEN p_fields ? 'notification_push' THEN (p_fields->>'notification_push')::boolean ELSE u.notification_push END,
    att_status = CASE WHEN p_fields ? 'att_status' THEN (p_fields->>'att_status')::varchar ELSE u.att_status END,
    att_prompted_at = CASE WHEN p_fields ? 'att_prompted_at' THEN (p_fields->>'att_prompted_at')::timestamptz ELSE u.att_prompted_at END,
    last_login_at = CASE WHEN p_fields ? 'last_login_at' THEN (p_fields->>'last_login_at')::timestamptz ELSE u.last_login_at END,
    onboarding_completed_at = CASE WHEN p_fields ? 'onboarding_completed_at' THEN (p_fields->>'onboarding_completed_at')::timestamptz ELSE u.onboarding_completed_at END,
    expert_title = CASE WHEN p_fields ? 'expert_title' THEN (p_fields->>'expert_title')::varchar ELSE u.expert_title END,
    expert_organization = CASE WHEN p_fields ? 'expert_organization' THEN (p_fields->>'expert_organization')::varchar ELSE u.expert_organization END,
    metadata = CASE
                 WHEN p_fields ? 'metadata' AND jsonb_typeof(p_fields->'metadata') = 'object'
...
-- ------------------------------------------------------------
-- start_conversation (Round 7 — atomic convo + participant rows)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- create_support_ticket (Round 7 — atomic ticket + first message)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- get_own_login_activity (Round 9 — self-serve login activity)
-- ------------------------------------------------------------
...
REVOKE ALL ON FUNCTION public.award_reading_points(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.award_reading_points(uuid) TO authenticated, service_role;

-- ------------------------------------------------------------
-- anonymize_user (Round 6 hardened: self-anonymize guard; cron-safe)
-- ------------------------------------------------------------
...
BEGIN
  ...
  -- and all non-matching sessions pass through.
  IF auth.uid() IS NOT NULL AND auth.uid() = p_user_id THEN
    RAISE EXCEPTION 'anonymize_user may not be self-invoked' USING ERRCODE = '42501';
  END IF;

  UPDATE users SET
    email = NULL, email_verified = false, email_verified_at = NULL,
    ...
    avatar_url = NULL, avatar_color = NULL, banner_url = NULL,
    date_of_birth = NULL, gender = NULL, country_code = NULL, timezone = NULL,
    referral_code = NULL, stripe_customer_id = NULL,
    last_login_ip = NULL, last_login_device = NULL,
    profile_visibility = 'private',
    show_activity = false, show_on_leaderboard = false, allow_messages = false,
    is_active = false,
    deleted_at = now(), deletion_completed_at = now(),
    notification_email = false, notification_push = false,
    updated_at = now()
  WHERE id = p_user_id;
...
-- ============================================================
-- ROUND 6 ACL LOCKDOWN — admin RPCs limited to service_role.
-- Bodies defined earlier in this rebuild file or in the earlier
-- numbered migrations. This block only tightens ACLs.
-- ============================================================
DO $acl$
DECLARE r record;
BEGIN
  FOR r IN
    ...
    WHERE p.pronamespace = 'public'::regnamespace
      AND p.proname IN (
        'grant_role','revoke_role','apply_penalty','resolve_appeal',
        'approve_expert_application','reject_expert_application',
        'send_breaking_news','anonymize_user',
        'schedule_account_deletion','cancel_account_deletion',
        'hide_comment','unhide_comment','resolve_report','mark_probation_complete'
      )
  LOOP
...
-- Final bump so clients refresh caches.
