...
### 2026-04-20 — T-019 partial sweep: 16 admin pages migrate inline role arrays to frozen Sets
- Files: `components/CommentThread.tsx`, `admin/reports/page.tsx`, `admin/recap/page.tsx`, `admin/kids-story-manager/page.tsx`, `admin/comments/page.tsx`, `admin/verification/page.tsx`, `admin/categories/page.tsx`, `admin/data-requests/page.tsx`, `admin/expert-sessions/page.tsx`, `admin/ad-placements/page.tsx`, `admin/ad-campaigns/page.tsx`, `admin/sponsors/page.tsx`, `admin/page.tsx`, `admin/permissions/page.tsx`, `admin/settings/page.tsx`, `admin/moderation/page.tsx`, `admin/users/[id]/permissions/page.tsx`, `api/expert-sessions/[id]/questions/route.js`, `api/expert-sessions/questions/[id]/answer/route.js`.
- Change: verbatim-matching inline arrays swapped to the frozen Sets exported from `lib/roles.js` (ADMIN_ROLES / EDITOR_ROLES / MOD_ROLES). Two files had locally-declared `MOD_ROLES` constants whose contents were semantically ADMIN_ROLES / EDITOR_ROLES — renamed + replaced with the canonical exports. admin/moderation line 120 swapped to ADMIN_ROLES; lines 27 and 121 kept (bespoke specialty-role and moderator-only subsets). T-019 stays open for the 12 remaining 2-wide `['owner', 'admin']` cases which need per-page product decisions (bug vs. intentional superadmin exclusion).
- Verify: tsc --noEmit exit 0; residual grep for 3+ wide role literals outside `lib/roles.js` = 0.

### 2026-04-20 — Batch B: plan_features + roles helpers + admin/users dropdowns + home FALLBACK_CATEGORIES + help prices (T-017, T-056, T-102, T-103; T-016/T-018/T-019 partial)
- Files: `web/src/lib/plans.js` (new `getPlanLimit()` + `getPlanLimitValue()` helpers, 60s cache); `web/src/lib/roles.js` (new `getRoles()` / `getRoleNames()` / `rolesUpTo()` / `rolesAtLeast()`); `web/src/app/bookmarks/page.tsx` (cap from DB); `web/src/app/story/[slug]/page.tsx` (cap from DB — 3 literal 10s replaced); `web/src/app/api/cron/send-push/route.js` (breaking-news daily cap from DB); `web/src/app/admin/users/page.tsx` (PLAN_OPTIONS + ROLE_ORDER removed, both DB-live); `web/src/app/page.tsx` (FALLBACK_CATEGORIES + FALLBACK_SUBCATEGORIES + interfaces deleted); `web/src/app/admin/story-manager/page.tsx` (CATEGORIES + SUBCATEGORIES const removed, DB-loaded state); `web/src/app/help/page.tsx` (price strings render from `plans.price_cents`).
- Change: same pattern as T-001 scoreTiers.ts. Built two helpers that cache plan-feature limits and role hierarchy for 60s, migrated the most-visible consumers. Bookmark cap and breaking-news daily cap now come from `plan_features` rows (admin/system edits take effect within a TTL). Home-page category bar reads strictly from DB — no more fake `fb-*` IDs filling gaps. admin/story-manager's category + subcategory dropdowns are DB-seeded and can accept any new category. admin/users' plan + role dropdowns both come from DB now.
- Residual: T-016 (admin/subscriptions + profile/settings still read PRICING/TIERS for upgrade UI), T-018 (admin/pipeline + admin/cohorts still hardcode categories), T-019 (19 admin pages still enumerate role arrays inline) — all three tasks left open with updated scope text pointing at remaining consumers. Helpers are in place; the sweep is mechanical.
- Verify: tsc --noEmit exit 0; grep `FREE_BOOKMARK_CAP` = removed-only; grep `FALLBACK_CATEGORIES` in page.tsx = 1 historical comment; grep `PLAN_OPTIONS|ROLE_ORDER` in admin/users = 0.
