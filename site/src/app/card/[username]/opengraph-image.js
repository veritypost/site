...
// if the target isn't paid / is private / doesn't exist, so crawlers
// don't get 500s.
export default async function Image({ params }) {
  ...
  const { data: target } = await supabase
    .from('users')
    .select('id, username, display_name, bio, avatar_color, verity_score, streak_current, profile_visibility, plans(tier)')
  ...
  const brandPlate = (
    <div style={{
      ...
      fontSize: 56, fontWeight: 800, fontFamily: 'system-ui, -apple-system, sans-serif',
    }}>
      Verity Post
    </div>
  );
  ...
  return new ImageResponse(
    (
      <div style={{
        ...
        <div style={{ fontSize: 20, fontWeight: 700, letterSpacing: 3, textTransform: 'uppercase', color: '#888' }}>
          Verity Post
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 28, marginTop: 40 }}>
          <div style={{
            ...
            color: '#fff', fontSize: 72, fontWeight: 700,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}>
            {initial(name)}
          </div>
        ...
        <div style={{ display: 'flex', gap: 32, marginTop: 'auto', paddingTop: 24, borderTop: '2px solid #e5e5e5' }}>
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <div style={{ fontSize: 16, color: '#666', fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase' }}>Verity Score</div>
            <div style={{ fontSize: 44, fontWeight: 800 }}>{score}</div>
          </div>
          {streak > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              <div style={{ fontSize: 16, color: '#666', fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase' }}>Streak</div>
              <div style={{ fontSize: 44, fontWeight: 800 }}>Day {streak}</div>
            </div>
          )}
          {topCategories.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
              <div style={{ fontSize: 16, color: '#666', fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase' }}>Top categories</div>
              <div style={{ fontSize: 22, marginTop: 4, display: 'flex', gap: 18 }}>
                {topCategories.map((c, i) => (
                  <span key={i}>{c.categories?.name || ''} {c.score}</span>
                ))}
              </div>
            </div>
          )}
