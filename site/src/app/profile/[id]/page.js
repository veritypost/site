...
const C = {
  ...
  text: '#111111', dim: '#666666', accent: '#111111', success: '#22c55e',
};
...
export default function PublicProfilePage() {
  ...
  useEffect(() => {
    ...
    async function fetchProfile() {
      ...
      const PUBLIC_USER_FIELDS = 'id, username, display_name, avatar_color, avatar_url, banner_url, bio, verity_score, created_at, profile_visibility, is_expert, expert_title, expert_organization, streak_current, streak_best, followers_count, following_count';
      let userData = null;
      ...
      userData._followingCount = followingCountData || 0;
      ...
      if (authUser) {
        ...
        const [followRes, blockRes] = await Promise.all([
          supabase.from('follows').select('id').eq('follower_id', authUser.id).eq('following_id', userData.id).maybeSingle(),
          supabase.from('blocked_users').select('id').eq('blocker_id', authUser.id).eq('blocked_id', userData.id).maybeSingle(),
        ]);
      ...
      const accuracy = totalQuizzes > 0
        ? Math.round(attemptList.reduce((sum, a) => sum + (a.total ? (a.correct / a.total) * 100 : 0), 0) / totalQuizzes)
        : 0;
      ...
      const [readingRes, quizRes, commentsRes] = await Promise.all([
        supabase.from('reading_log').select('id, created_at').eq('user_id', userData.id).order('created_at', { ascending: false }).limit(5),
        supabase.from('quiz_attempts').select('article_id, attempt_number, is_correct, created_at').eq('user_id', userData.id).is('kid_profile_id', null).order('created_at', { ascending: false }).limit(100),
        supabase.from('comments').select('id, body, created_at').eq('user_id', userData.id).order('created_at', { ascending: false }).limit(5),
      ]);

      const activityItems = [];
      (readingRes.data || []).forEach(r => activityItems.push({
        id: 'r-' + r.id, text: 'Read an article', time: r.created_at ? new Date(r.created_at).toLocaleDateString() : '', icon: '',
      }));
      ...
      (commentsRes.data || []).forEach(c => activityItems.push({
        id: 'c-' + c.id, text: 'Posted a comment', time: c.created_at ? new Date(c.created_at).toLocaleDateString() : '', icon: '',
      }));
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
      <header style={{
        ...
        alignItems: 'center', justifyContent: 'space-between', padding: '0 20px', height: 56,
      }}>
        <a href="/" style={{ fontSize: 17, fontWeight: 700, color: C.text, textDecoration: 'none', letterSpacing: '-0.03em' }}>verity post</a>
        ...
        <div style={{ textAlign: 'center', padding: '32px 20px 24px' }}>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
            <button onClick={handleFollow} style={{
              padding: '9px 24px', borderRadius: 24, fontSize: 13, fontWeight: 600, cursor: 'pointer',
            ...
            <button style={{
              padding: '9px 24px', borderRadius: 24, fontSize: 13, fontWeight: 600, cursor: 'pointer',
              background: C.card, color: C.dim, border: '1px solid ' + C.border,
            }}>Message</button>
