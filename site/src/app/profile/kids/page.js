...
// D12/D34/D44 — parent-facing kid dashboard. Trial flow + CRUD +
// household rollups + pause toggle.
//
// Chunk 2 pass: rebuilt as a hero surface. Top row is a 4-up KPI card
// (rolling 7d: articles, minutes, quizzes passed, longest streak).
// Trial has its own hero banner with a prominent countdown + upgrade
// CTA. Per-kid cards carry status pills + Pause/Resume.
...
export default function ParentKidsPage() {
  ...
  const [form, setForm] = useState({ display_name: '', avatar_color: COLOR_OPTIONS[0], pin: '', pinConfirm: '', date_of_birth: '', parent_name: '', consent_ack: false });
  const [saving, setSaving] = useState(false);
  const [pendingRemove, setPendingRemove] = useState(null);
  const [removeBusy, setRemoveBusy] = useState(false);
  const [pauseBusy, setPauseBusy] = useState(null); // kid id currently toggling

  async function load() {
    setLoading(true); setError('');
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) { setLoading(false); return; }
    const [meResult, kidsRes, trialRes, kpiRes] = await Promise.all([
      supabase.from('users').select('id, username, kid_trial_used, kid_trial_ends_at, plans(tier)').eq('id', user.id).maybeSingle(),
      fetch('/api/kids').then(r => r.json()).catch(() => ({ kids: [] })),
      fetch('/api/kids/trial').then(r => r.json()).catch(() => ({})),
      fetch('/api/kids/household-kpis').then(r => r.ok ? r.json() : null).catch(() => null),
    ]);
    const meRow = meResult?.data;
    setMe(meRow);
    setTier(meRow?.plans?.tier || 'free');
  ...
  const isFamilyPlan = tier === 'verity_family' || tier === 'verity_family_xl';
  const maxKids = tier === 'verity_family' ? 2 : tier === 'verity_family_xl' ? 4 : 0;
  ...
  const canCreateMore = isFamilyPlan ? kids.length < maxKids : (!trial?.kid_trial_used && kids.length === 0);

  async function createKid(asTrial) {
    setError(''); setFlash('');
  ...
  async function confirmRemoveKid() {
    ...
    setRemoveBusy(true);
    try {
      const res = await fetch(`/api/kids/${pendingRemove.id}`, { method: 'DELETE' });
  ...
  async function togglePause(kid) {
    if (pauseBusy) return;
    setPauseBusy(kid.id);
    setError(''); setFlash('');
  ...
  return (
    <div style={{ maxWidth: 820, margin: '0 auto', padding: '24px 16px 80px' }}>
      ...
      <div style={{ fontSize: 12, color: C.dim, marginBottom: 16 }}>
        Kid profiles are completely private — not in search, not on leaderboards, not visible to anyone outside your family (D12).
      </div>
      ...
      {/* Trial hero — takes precedence over KPI row until converted. */}
      {trialActive && <TrialHero endsAt={trial.kid_trial_ends_at} />}
      {trialExpired && <TrialExpiredHero />}

      {/* Household KPIs. Only shown when there's a family plan OR trial
          is active (otherwise the row is empty of data). */}
      {(isFamilyPlan || trialActive) && (
        <KpiRow kpis={kpis} />
      )}
      ...
      {/* Empty state for family-plan parents with no kids yet */}
      {kids.length === 0 && isFamilyPlan && (
        <div style={{ background: C.card, border: `1px dashed ${C.border}`, borderRadius: 14, padding: 24, textAlign: 'center', marginBottom: 16 }}>
          ...
          <p style={{ fontSize: 13, color: C.dim, margin: '0 0 12px' }}>Add up to {maxKids} kid profile{maxKids === 1 ? '' : 's'}. Each one gets private reading, quizzes, and expert sessions.</p>
        </div>
      )}

      {/* Upsell for non-family, non-trial users */}
      {!isFamilyPlan && !trialActive && !trialExpired && !trial?.kid_trial_used && kids.length === 0 && (
        <div style={{ background: '#fffbeb', border: `1px solid ${C.warn}`, borderRadius: 12, padding: 16, marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 12, color: C.text, marginTop: 4 }}>
            One kid profile, 7 days, no card. Convert to Verity Family to keep going — the kid&apos;s progress carries over.
          </div>
      ...
      {/* Add button */}
      {!showForm && canCreateMore && isFamilyPlan && (
        <button onClick={() => setShowForm('full')} style={{ padding: '9px 18px', borderRadius: 8, border: `1px dashed ${C.border}`, background: 'transparent', color: C.text, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
          + Add kid profile ({kids.length}/{maxKids})
        </button>
      )}
      {!canCreateMore && isFamilyPlan && (
        <div style={{ fontSize: 12, color: C.dim }}>You&apos;ve added the maximum ({maxKids}). Family XL supports 4 kids.</div>
      )}
      ...
      <div style={{ marginTop: 28, display: 'flex', gap: 16, flexWrap: 'wrap' }}>
        <a href="/profile/family" style={{ color: C.accent, fontSize: 13, fontWeight: 700, textDecoration: 'none' }}>Family dashboard →</a>
        <a href="/kids/expert-sessions" style={{ color: C.accent, fontSize: 13, fontWeight: 700, textDecoration: 'none' }}>Expert sessions →</a>
      </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 16, fontWeight: 800, color: C.danger, marginBottom: 4 }}>
        Trial ended — kid profile is frozen
      </div>
...
function KidCard({ kid, pauseBusy, onDashboard, onPauseToggle, onDelete }) {
  const paused = !!kid.paused_at;
  const trial = !!kid.metadata?.trial;
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 4 }}>
            {trial && <StatusPill label="Trial" color={C.warn} />}
...
function CreateKidForm({ form, setForm, saving, mode, onCancel, onSubmit }) {
...
const inputStyle = { width: '100%', padding: '8px 10px', borderRadius: 8, border: '1px solid #e5e5e5', fontSize: 13, outline: 'none', fontFamily: 'inherit' };
