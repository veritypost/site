...
export default function SignupPage() {
  ...
  const passwordMeetsPolicy = PASSWORD_REQS.every(r => r.test(password));
  ...
  const checkEmailAvailability = async (value) => {
    ...
    if (!v || !v.includes('@')) { setEmailCheck({ status: 'idle' }); return; }
    setEmailCheck({ status: 'checking' });
    try {
      const res = await fetch(`/api/auth/check-email?email=${encodeURIComponent(v)}`);
      if (!res.ok) { setEmailCheck({ status: 'idle' }); return; }
      const body = await res.json();
      if (!body.checked) { setEmailCheck({ status: 'idle' }); return; }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h1 style={{ fontSize: '26px', fontWeight: '700', color: C.text, margin: '0 0 6px 0' }}>Join Verity Post</h1>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: '14px' }}>
            ...
            {emailCheck.status === 'taken' && <div style={{ fontSize: 11, color: '#ef4444', marginTop: 4 }}>An account already exists for that email. <a href={`/login?email=${encodeURIComponent(email)}`} style={{ color: '#ef4444', fontWeight: 600, textDecoration: 'underline' }}>Log in instead</a>.</div>}
          </div>
          ...
          {password && (
            <div style={{ marginBottom: '14px' }}>
              <div style={{ display: 'flex', gap: '4px', marginBottom: '5px' }}>
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} style={{ flex: 1, height: '3px', borderRadius: '99px', backgroundColor: i <= strength.bars ? strength.color : C.border, transition: 'background-color 0.2s' }} />
                ))}
              </div>
          ...
          <button type="submit" disabled={loading || !agreedToTerms || !ageConfirmed || !passwordMeetsPolicy || mismatch}
            style={{ width: '100%', padding: '13px', fontSize: '15px', fontWeight: '600', color: '#fff', backgroundColor: (loading || !agreedToTerms || !ageConfirmed || !passwordMeetsPolicy || mismatch) ? '#cccccc' : C.accent, border: 'none', borderRadius: '10px', cursor: (loading || !agreedToTerms || !ageConfirmed || !passwordMeetsPolicy || mismatch) ? 'not-allowed' : 'pointer', marginBottom: '20px', fontFamily: 'inherit' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '14px' }}>
          ...
          <span style={{ fontSize: '12px', color: C.dim, whiteSpace: 'nowrap' }}>or sign up with</span>
          <div style={{ flex: 1, height: '1px', backgroundColor: C.border }} />
        </div>
        ...
        <p style={{ textAlign: 'center', fontSize: '13px', color: C.dim, margin: 0 }}>
          ...
          <a href="/login" style={{ color: C.accent, fontWeight: '600', fontSize: '13px', textDecoration: 'none' }}>
            Sign in
          </a>
        </p>
      </div>
