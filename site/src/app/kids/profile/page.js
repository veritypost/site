...
// /kids/profile — kid's own profile in kid mode. Matches iOS KidProfileView:
// identity header in kid theme colour → 4 chunky stat cards (Articles,
// Day Streak, Badges, Verity Score) → badges grid → kid-safe saved
// articles (parent's bookmarks filtered to is_kids_safe=true) → Switch
// profile + Exit kid mode actions.
//
// Chunk 7 consolidation: the exit-PIN modal lives on KidTopChrome. This
// page calls `useKidChrome().openExitPin(intent)` rather than hosting a
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: ach } = await supabase
        ...
        .select('id, achievement_id, earned_at, achievements(key, display_name, icon_name)')
  ...
  return (
    <div>
      ...
      <div style={{ maxWidth: KID.space.maxWidth, margin: '0 auto', padding: `0 16px ${KID.space.sectionGap}px` }}>
          ...
          {badges.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: KID.space.rowGap }}>
              {badges.map((b) => (
                <Badge
                  key={b.id}
                  name={b.achievements?.display_name || b.achievements?.key || 'Badge'}
                />
              ))}
            </div>
          )}
        ...
        {/* Bottom actions route through the chrome context. */}
