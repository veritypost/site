...
// GET /api/recap — list this week's recaps visible to the caller.
// D36: Verity+ only. Free users get an empty list so the home card
...
export async function GET(request) {
  ...
  if (!paid) return NextResponse.json({ recaps: [], paid: false });
  ...
  let q = service.from('weekly_recap_quizzes')
  ...
  if (ids.length > 0) {
    const { data: a } = await service
      ...
      .eq('user_id', user.id).in('recap_quiz_id', ids);
    attempts = a || [];
  }
