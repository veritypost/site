...
//
// Auth: requireRole('admin') (covers admin, superadmin, owner via the
// hierarchy in lib/auth.js). Service client is used for the writes —
// same pattern as the sibling roles route — because RLS on the perms
// tables is intentionally restrictive. The requireRole check is the
...
export async function POST(request, { params }) {
  // Auth gate. requireRole throws on unauthenticated or insufficient role.
  let actor;
  try {
  ...
  const { error: bumpErr } = await service
    .from('users')
    .update({
      perms_version: (targetUser.perms_version || 0) + 1,
      perms_version_bumped_at: new Date().toISOString(),
    })
    .eq('id', targetUserId);
  if (bumpErr) {
