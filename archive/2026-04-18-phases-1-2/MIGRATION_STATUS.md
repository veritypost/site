...
Date: 2026-04-18
Scope: all 23 hard data-layer bugs from `05-Working/ADMIN_AUDIT.md`.

Per-page status:

| Page | Issue | Fix |
|---|---|---|
| `admin/users/[id]/permissions/page.js` | L129 ambiguous `user_roles(...)` embed (PGRST201) | Added `!fk_user_roles_user_id` hint; also added `!fk_user_roles_role_id` on the auth-gate user_roles query |
| `admin/support/page.js` | L190 ambiguous `users(...)` embed on `support_tickets` (2 FKs) | Qualified with `users:users!fk_support_tickets_user_id` + red error banner |
| `admin/moderation/page.js` | L59 wrong FK hint `user_warnings_user_id_fkey` | Replaced with `users:users!fk_user_warnings_user_id` |
| `admin/analytics/page.js` | `quiz_attempts.passed` missing; `articles.avg_read_time` & `quiz_pass_rate` missing | Swapped to `is_correct`; removed dead columns; platform-wide pass rate now derived from attempts |
| `admin/pipeline/page.js` | `.order('date', ...)` on `pipeline_costs` | Changed to `.order('created_at', ...)` |
| `admin/email-templates/page.js` | Filter on non-existent `category` column | Replaced with Active/Disabled status filter + red error banner |
| `admin/feeds/page.js` | `rss_feeds` table + legacy columns | Rewritten against `feeds` with Blueprint v2 columns (`is_active`, `error_count`, `last_polled_at`, etc.) and derived health status |
| `admin/ingest/page.js` | `story_clusters` table | Rewritten against `feed_clusters` + `feed_cluster_articles` with keyword-fallback display |
| `admin/webhooks/page.js` | Retry wrote to non-existent `webhooks` table | Retry now updates `webhook_log.processing_status` + `retry_count` |
| `admin/breaking/page.js` | INSERT used `text/story/sent_by/target` (none exist) | Rewritten insert against real `articles` columns; audience lives in `metadata.target` |
| `admin/subscriptions/page.js` | `invoices.refund_status` column missing; non-standard sub status | Refund state stored in `invoices.metadata.refund_status`; statuses canonicalised to `cancelled`/`active` with `metadata.pending_stripe_sync` |
| `admin/system/page.js` | Rate limits read/written to `feature_flags` with wrong columns | Repointed at `rate_limits` table; UI window/scope translated to `window_seconds`/`scope` |

All 12 pages also surface query errors in a red banner (no more silent `loading=false`).

### TypeScript install

- `site/src/types/database.ts` generated (8918 lines, 273 KB) via Supabase MCP + CLI.
- `site/src/types/database-helpers.ts` adds `Tables<T>`, `TableInsert<T>`, `TableUpdate<T>`, `Enums<T>`, `DbClient`.
- `site/src/lib/supabase/client.js` + `server.js` converted to `.ts`, client typed as `SupabaseClient<Database>`.
- `site/tsconfig.json` added (allowJs:true, strict:false — existing JS admin pages still compile).
- `npm run types:gen` regenerates the types from the live VP project.
