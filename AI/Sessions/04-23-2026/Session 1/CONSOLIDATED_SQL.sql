...
-- =============================================================================
--
-- One SQL block covering every pending DB change from Wave 1 + Wave 2.
-- Paste the ENTIRE file in Supabase SQL editor in a single transaction.
--
-- BUNDLE CONTENTS
--   schema/144_drop_orphan_tables.sql — Drops 9 confirmed-dead 0-row tables
--                                       (Wave 2 Stream 3, body inlined below).
--
-- ALREADY APPLIED (informational, not re-run here):
--   schema/106_kid_trial_freeze_notification.sql — Verified live in prod
--     via MCP `pg_get_functiondef` matches file 1:1. FIX_SESSION_1 entry
--     `00-M` is stale; mark SHIPPED.
--   schema/142_profile_leaderboard_consensus_ship.sql — Shipped 2026-04-22
--     (`62077f0`).
--
-- CODE-ONLY (no SQL impact):
--   `@admin-verified` marker bumps (77 files) — awaiting commit per
--     OWNER_QUESTIONS.md §6.4.
--   Color-token sweep (this session, C10).
--
-- =============================================================================
-- VERIFICATION BEFORE APPLYING
-- =============================================================================
--
-- Run this one-liner first to confirm prod still matches Wave 2 Stream 3
-- findings (should return exactly 9 rows, all with rows = 0):
--
--   SELECT table_name,
...
--
-- After commit, recovery requires PITR. The 9 tables had 0 rows so nothing
-- of value would be lost; the only cost is recreating the schema. The
-- original DDL for each is preserved in `schema/reset_and_rebuild_v2.sql`.
-- Cheaper to re-author from there than to PITR if rollback is ever needed.
--
-- =============================================================================
-- AFTER APPLYING — AGENT FOLLOW-UPS (NEXT SESSION)
-- =============================================================================
--
-- 1. Author `schema/144_drop_orphan_tables.sql` containing the migration
--    body above (matches what was actually applied).
