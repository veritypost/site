...
```
CURRENT: Discovery scraper layer — Phase B shipped. /api/newsroom/ingest/run now fans out to RSS + scrape_html + scrape_json consumers in parallel via Promise.allSettled. New feeds.extraction_config jsonb column drives per-feed JSON field-mapping with env-var-allow-listed Authorization (NEWSAPI_KEY / NEWSDATA_KEY / MEDIASTACK_KEY / GNEWS_KEY) and per-env-var host binding (key only resolves on its bound vendor host). /admin/feeds drawer gains an "Extraction config" section (gated to feed_type='scrape_json') with Defaults dropdown + Save + Test (read-only test endpoint, rate-limited 10/60s/actor, no writes to discovery_items). Save endpoint redacts inline header + secret-shaped query_param values in the audit-log payload.
SHAPE:   fix-cluster (admin chrome + pipeline-data — not on §8.4 lock list, free to edit)
TARGET:  /api/newsroom/ingest/run · web/src/app/api/newsroom/ingest/run/route.ts (3rd fanout) + web/src/lib/pipeline/extraction-config.ts (NEW) + web/src/lib/pipeline/scrape-json.ts (NEW) + web/src/app/api/admin/feeds/[id]/extraction-config/route.ts (NEW) + web/src/app/api/admin/feeds/[id]/extraction-config/test/route.ts (NEW) + web/src/app/admin/feeds/_ExtractionConfigEditor.tsx (NEW) + web/src/app/admin/feeds/page.tsx (additive drawer section + import) + supabase/migrations/20260504170000_feeds_extraction_config.sql
STATUS:  pending-prod-confirm
NOTES:   CLI-side gates green — tsc 0 errors, lint 0 errors on touched files, build clean (`✓ Compiled successfully`). Adversary review surfaced 9 findings; 5 fixed in slice (env-var → host binding to prevent URL-pivot exfiltration, top-level extraction_config strict-key allowlist, dot-path proto-pollution segment guard `__proto__`/`constructor`/`prototype`, audit-log redaction of inline secret values, in-memory rate-limit map opportunistic eviction, editor remount-per-feed via `key={selected.id}`), 4 deferred to Phase C (vendor-echo theoretical leak, save vs clear-errors UX coupling, stale "Saved" status, audit-log column existence verification). Active feed_type distribution post-Phase-A migration unchanged: 100 feed + 23 rss + 96 scrape_html + 33 scrape_json (252 total). All 33 scrape_json rows currently surface as "unconfigured" (extraction_config = `{}` default) until operator fills configs via /admin/feeds drawer. Env vars NEWSAPI_KEY / NEWSDATA_KEY / MEDIASTACK_KEY / GNEWS_KEY are NOT yet set in Vercel — Owner action.

Known limits / Phase C followups:
  - Phase A items still open: scrape_html zero-results doesn't alert; concurrent-run error_count race; article-URL heuristic accepts year-archive paths.
  - Vendor-echo leak: a malicious vendor could place env-var values in `title` field — addressed in practice by host binding (op can't pivot URL to attacker), but defense-in-depth scrub on Test response could be Phase C.
  - Editor "Saved at HH:MM:SS" status persists while textarea is dirty — Phase C polish.
  - Save with `{}` clear path doesn't auto-reset error_count — operator clicks "Clear errors" separately.
  - JSONPath syntax (`$.results[*]`) not supported — dot-paths only. Vendors that need it = file Phase C.
  - No automated cron for scrape_json sources yet — manual-trigger via /admin/newsroom Discovery → "Refresh feeds".

Smoke checklist (owner runs on veritypost.com/admin/newsroom + /admin/feeds while signed in as admin@veritypost.com):
  1. /admin/feeds → click any scrape_json row → drawer shows "Extraction config" section with Defaults dropdown + JSON textarea + Save + Test buttons. Section ABSENT for rss / scrape_html / feed rows.
  2. /admin/feeds drawer → pick a row whose URL is on newsapi.org → choose "NewsAPI" from Defaults dropdown → textarea pre-fills with the NewsAPI shape including `${NEWSAPI_KEY}` placeholder. Click Save → status line confirms saved. Refresh page → drawer shows the saved config.
  3. /admin/feeds drawer → click Test → if NEWSAPI_KEY is set in Vercel env: status shows "Tested — N articles. (Showing first 5)" with a list of article URLs. If NEWSAPI_KEY unset: status shows the error reason. NO writes to discovery_items either way (confirm via Story Manager total count).
  4. /admin/newsroom Discovery → "Refresh feeds" → toast confirms run; response includes feedsByType.scrape_json { polled, succeeded, failed, unconfigured } and itemsBySource.scrape_json. Unconfigured count starts at 33 and decrements as configs are saved.
  5. /admin/feeds → after running #4, the configured scrape_json row shows last_polled_at advanced AND non-zero Items/24h (if Test step worked). Unconfigured rows show last_polled_at advanced and zero items, no error_count change.
  6. Smoke env-var → host binding: paste a NewsAPI config but change the feed.url to https://example.com → Test button reports "invalid extraction_config — fix and re-test" OR scrape returns no articles + Vercel log shows `[scrape_json.config_unresolved]` (key did not resolve because host doesn't match newsapi.org).
  7. /admin/feeds drawer → paste an inline literal in headers (e.g. `{"X-Api-Key":"sk_live_abc123"}`) → Save persists to feeds.extraction_config but admin_audit_log entry shows `headers: { "X-Api-Key": "[INLINE-VALUE-REDACTED]" }` (verify by querying admin_audit_log via /admin or Supabase).
```
...
- **Following page is removed.** `/following` is deleted; reading-history / followed-stories merge into `/browse` (UI TBD). Nav bar entry removed. (Owner Change #2)
