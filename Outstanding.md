...
**Best-practice review applied 2026-05-12 (4-investigator pass + adversary):**

- **Q-NEW6 reversed.** The original "auto-push You on appear" was a mechanical port of web mobile's pattern (web has no swipe-back gesture, so it auto-pushes to avoid a tap). iOS has swipe-back for free, and the native iOS pattern (Apple Health/Wallet/News/Settings) is **dashboard inline at top of a master list, sections below**. Final: iPhone Profile = `NavigationStack` with the section list as root; "You" rendered inline-expanded at top (current ProfileView dashboard content moves into a `YouSectionView` rendered as the first list item, not a destination); the other 20 sections render as `NavigationLink` rows below. No auto-push, no top-bar Sections menu. Reaching Activity = scroll + 1 tap. Reaching the dashboard = already there. iPad: stock `NavigationSplitView` with same section list as sidebar, default selection You.
- **Q-E2 confirmed safe by code.** iOS deployment target is 17.0 (project.pbxproj line 6) — `NavigationSplitView` is hardened at this version. Zero existing usage in the codebase (no host conflicts to inherit). Single `TabView` (`ContentView.swift:241`) hosts each tab's own `NavigationStack`; `ProfileView` carries no nested nav. `HomeView` already does structural size-class branching as precedent (`isGridMode` 2-col grid). Direct integration; no spike PR required.
- **Q-E5 concretized.** Web's `LinkOutSection` for `id: "help"` (`ProfileApp.tsx:495-504`) renders 2 rows: "Open help center" → `/help` (real FAQ page at `web/src/app/help/page.tsx`, 7 FAQs + signup CTA + support email), "Contact support" → `/contact` (11-topic form). iOS skips the email field — every iOS session is signed-in (`api/support` uses `requireAuth(supabase)`), so identity flows via the bearer token. Server route accepts category as a free string and forwards to `create_support_ticket` RPC, so all 11 web values work as-is.
- **Session 4 splits.** Adversary scope verdict (verified by code-counting agent against actual line counts): `ProfileView.swift` is 1970 lines, `SettingsView.swift` is 4436, web's `ProfileApp.tsx` is 576 + 21 child sections; ~14 of 21 sections have iOS analogues already in the SettingsView pair. Honest day-count for shell-only port: 4–5 days. Bundling rail rendering and snapshot tests pushes to 9–10 days (per scope agent: rail = 1.5–2 days because `storyContent` shares `scrollOffset` state via `ArticleScrollOffsetKey` at `StoryDetailView.swift:232`, can't HStack-wrap; snapshots = wire `swift-snapshot-testing` + create XCTest target + ~25–30 baselines = 2–3 days). Final: Session 4 = Profile shell only. Rail rendering is its own session. Snapshot tests are their own session. Per the locked Session 3 part 1 / part 2 precedent, Session 4 itself splits into part 1 (this commit, Help parity in About hub + decision-locking) and part 2 (the actual shell port).
...
- `SettingsView.swift:3986-4080` — `FeedbackSheet` rewritten. New `SupportTopic` model + 11 topics (`account / billing / bug / content / feature / kids / expert / feedback / accessibility / appeal / other`) mirroring web `contact/page.tsx:20-32`. Picker swapped from `.segmented` (overflowed past 3 options) to a `Menu { Picker { ForEach } }` pattern — native iOS idiom for long option lists, displayed as a 44pt-min row showing the current selection with chevron affordance. Sheet title now "Contact support" (was "Send feedback"). Section header renamed "Topic" / "Your message". `submit()` body unchanged — server (`api/support` route) accepts the new category strings as-is via `create_support_ticket` RPC.
- `SettingsView.swift:1134-1152` — `aboutRows` gains a "Help center" row above "Contact support", icon `questionmark.circle.fill`, kind `.external(URL("https://veritypost.com/help"))`. Reuses the existing `Link`-based external pattern that Privacy policy / Terms of service already use; opens in Safari (consistent with surrounding rows). "Send feedback" row renamed "Contact support" to match the sheet's new title.
- `Outstanding.md` — Q-NEW6 reversed (best-practice review) to inline-You-at-top, no auto-push; Q-E5 concretized with verbatim web mapping; Group E preamble gained a Best-practice-review block; Session 4 split into part 1 (this) + part 2 (full shell port).

**Cross-platform footnote:**
- Web: N/A — Help section is the source of truth being mirrored to iOS, no web change.
...
- **CONCRETIZED 2026-05-12 (best-practice review against web code).** Web's Help section is 2 rows via `LinkOutSection` at `ProfileApp.tsx:495-504`: "Open help center" → `/help` (real FAQ page at `web/src/app/help/page.tsx`, ~7 FAQs + signup CTA + `support@veritypost.com` fallback) and "Contact support" → `/contact` (11-topic form, `web/src/app/contact/page.tsx:20-32`). No external services (no Statuspage / Discord / etc). iOS skips web's email field — every iOS session is signed-in (`api/support` route at `web/src/app/api/support/route.js:41-62` calls `requireAuth(supabase)`, identity flows via bearer token). The 11 topics: Account Issue, Billing & Subscription, Report a Bug, Content Concern, Feature Request, Kids Mode, Expert Verification, App Feedback, Accessibility, Ban Appeal, Other. Server accepts category as a free string and forwards to `create_support_ticket` RPC, so all 11 web values work as-is.
...
- **REVISED 2026-05-12 (best-practice review) — Inline-You at top of list, no auto-push.** The original answer was a mechanical port of web mobile's pattern. Web auto-pushes because mobile web has no swipe-back gesture. iOS has swipe-back for free, and the native iOS pattern (Apple Health, Wallet, News, Settings) is dashboard-inline-at-top + sections below. Final: iPhone Profile = `NavigationStack` rooted at the section list. The first list item is "You" rendered inline-expanded (current ProfileView dashboard cards: hero, stats, social, quick actions, previews — moved into a `YouSectionView` invoked as a list section, not a destination). The other 20 sections render as `NavigationLink` rows below. Reaching the dashboard = already there on tab tap. Reaching Activity = scroll a few items + 1 tap. iPad: stock `NavigationSplitView`, default selection You.
...
**SHIPPED 2026-05-12.** Investigator audit (re-verified by sole-implementer pass): four consumers read `signup_source`, all correctly tagged as durable origin —
- `web/src/app/api/admin/access-requests/[id]/approve/route.ts:84,216`
- `web/src/app/api/admin/access-requests/bulk-approve/route.ts:112,234`

Both files stamp `access_requests.consumption_source` from `auth.users.user_metadata.signup_source` (durable origin), which is the right field for that intent. **No mis-reads found.** Zero consumers read `audit_log.metadata.client` outside of write paths and GDPR data-export dumps (which return whole rows, not filtered/aggregated). No funnel, cohort, retention, growth, or admin dashboard queries currently consume either field, so there was nothing to fix. Admin pages reference `admin_audit_log` (a different table with no `metadata.client`). The risk the item was created to prevent (a future dashboard conflating the two) is now headed off by a canonical reference doc at `web/src/app/admin/_docs/ANALYTICS_FIELDS.md` — covers the distinction with a side-by-side table, ✅/❌ SQL examples, the `signup_complete` edge case (audit row's `metadata.client` equals origin by definition for a user's first login), and the schema-location footnote (`signup_source` lives only in `auth.users.raw_user_meta_data` JSONB; `metadata.client` lives in `public.audit_log.metadata`). Underscore-prefixed `_docs/` folder keeps it out of Next.js routing. Grep-discoverable for future devs/agents.
