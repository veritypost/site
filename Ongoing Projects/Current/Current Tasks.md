...
4. **Apply `schema/188_fix_billing_cancel_frozen_user.sql` via Supabase dashboard** (B17) — migration written 2026-04-26; owner must apply manually. Fixes frozen users having no recovery path to cancel their subscription. Migration 157 introduced a silent no-op (`skipped=true`) instead of actually cancelling — the subscription was never marked cancelled. Migration 188 removes the frozen-user short-circuit so cancellation proceeds regardless of freeze state. After apply, verify a frozen user's cancel call returns `subscription_id` + `grace_ends_at`, not `skipped=true`.
